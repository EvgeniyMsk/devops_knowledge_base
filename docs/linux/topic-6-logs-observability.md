...
**Цель:** быстро находить причину проблем: journald, rsyslog, logrotate, логи ядра, auditd (база). Уметь найти, почему сервис не стартует, настроить logrotate и отфильтровать логи за конкретный период.

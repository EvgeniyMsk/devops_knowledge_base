...
**Цель:** понимать, почему трафик «не ходит»: IPv4/IPv6, CIDR и подсети, частные и публичные адреса, NAT, таблица маршрутизации и статическая маршрутизация. Уметь считать подсети вручную и объяснять, почему под в Kubernetes недоступен извне без Service/Ingress.

...
**Цель:** понимать причины задержек, таймаутов и повторных передач: как устроен TCP (handshake, завершение, окно, MSS, ретрансмиты, TIME_WAIT/CLOSE_WAIT, Nagle и Delayed ACK), когда и зачем используется UDP, и что такое QUIC (HTTP/3).

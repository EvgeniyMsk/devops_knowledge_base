...
---
Компоненты Control Plane (API, etcd, scheduler, controller-manager) и узла (kubelet, kube-proxy, CNI, CSI, CRI), а также практический запуск кластера (kind, k3s, kubeadm).
...
| Термин | Определение |
|--------|-------------|
| **Кластер** | Набор нод (узлов), управляемых единым Control Plane; в кластере разворачиваются поды и сервисы. |
| **Node (нода, узел)** | Машина (VM или bare metal), на которой работают kubelet, kube-proxy и container runtime; поды планируются на ноды. |
| **Control Plane** | Управляющая плоскость кластера: API-сервер, etcd, scheduler, controller-manager; хранит состояние и принимает решения о размещении подов. |
| **Namespace (Kubernetes)** | Виртуальное разделение ресурсов внутри кластера (Pod, Service, ConfigMap и т.д.); изоляция по командам/окружениям и область действия RBAC. |
| **Reconciliation** | Цикл «приведения текущего состояния к желаемому»: контроллер сравнивает spec объекта с реальностью и создаёт/удаляет дочерние объекты. |
| **Static Pod** | Под, управляемый kubelet напрямую по манифесту с диска ноды (не через API-сервер); используется для компонентов control plane. |

...
---
Необходимо понимать, почему Kubernetes работает именно так: Linux (namespaces, cgroups, firewall), сеть (TCP/IP, DNS, NAT, балансировка L4/L7, TLS/mTLS) и контейнеры (образы, ENTRYPOINT/CMD, multi-stage, безопасность образов, OCI runtime). Без этого сложно осознанно работать с подами, сервисами и Ingress.
...
| Термин | Определение |
|--------|-------------|
| **Namespace (Linux)** | Механизм изоляции ядра Linux: процессы, сеть, монтирования и т.д. видят только свои ресурсы; контейнер — процесс(ы) в отдельном наборе namespace'ов. |
| **Cgroups** | Контрольные группы ядра Linux: ограничивают и учитывают использование CPU, памяти и I/O группой процессов; в Kubernetes задают limits/requests контейнера. |
| **Pod** | Минимальная единица развёртывания в Kubernetes: один или несколько контейнеров с общей сетью (один IP) и томами; планируется на одну ноду. |
| **Service** | Абстракция стабильного доступа к набору подов (виртуальный IP или DNS-имя); типы: ClusterIP, NodePort, LoadBalancer, Headless. |
| **Ingress** | Объект Kubernetes для L7-маршрутизации внешнего трафика по Host и path на внутренние Service; обрабатывается Ingress Controller. |
| **OCI (Open Container Initiative)** | Стандарт формата образа и runtime контейнеров; образы Docker и containerd совместимы с OCI. |
| **CRI (Container Runtime Interface)** | Интерфейс между kubelet и контейнерным runtime (containerd, CRI-O); kubelet не привязан к конкретному runtime. |
| **L4 / L7** | L4 — балансировка по IP и порту; L7 — по протоколу приложения (HTTP host/path, заголовки). |

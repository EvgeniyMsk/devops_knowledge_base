...
**Цель:** уверенно работать с хранилищем в кластере: PersistentVolume (PV) и PersistentVolumeClaim (PVC), процесс связывания, статическое и динамическое выделение, StorageClass (AccessModes, ReclaimPolicy), CSI (динамическое выделение, снимки), StatefulSet + PVC для сохранности данных. Best practices: выбор типа хранилища, производительность, безопасность, резервное копирование, мониторинг; комплексный production-пример (PostgreSQL). В разделе — примеры манифестов с комментариями и рекомендации для production.

...
**Цель:** проектировать отказоустойчивые системы в Kubernetes: автомасштабирование (HPA, VPA, Cluster Autoscaler, KEDA), устойчивость к сбоям — probes (liveness, readiness, startup), PodDisruptionBudget, размещение подов (Affinity / Anti-affinity, Taints и Tolerations). В разделе — примеры манифестов с комментариями и best practices для production.

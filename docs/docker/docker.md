# Docker
Docker — это платформа с открытым исходным кодом для автоматизации разработки, доставки и развёртывания приложений. Основная идея — создание стандартного и предсказуемого окружения, где приложения могут работать независимо от операционной системы или инфраструктуры. 
Docker позволяет «упаковать» приложение со всем окружением и зависимостями в контейнер — изолированное пространство, которое позволяет запускать приложения отдельно от основной системы. Контейнер включает в себя код приложения, системные библиотеки, переменные окружения и конфигурационные файлы — всё, что нужно для работы программы. 
